...
public class UserActivity extends Activity {
	...
	public void onCall(View view) {
		...
		if(!isCallingCab && address.length() > 0) {
			timeout_counter = 0;
			curTxnid = "";
			requestAPIFunction3(address);
			isCallingCab = true;
	...
	void reset() {
		handler.post(new Runnable() {
			public void run() {
				...
				((TextView)findViewById(R.id.textViewStatus)).setText(R.string.searching);
				showLoadingArea(false);
			}
		});
	}
	...
	// API calls
	void requestAPIFunction3(String address) {
		String encoded_address = URLEncoder.encode(address);
	...
	void alertPickuped(final double distance) {
		...
		((TextView)findViewById(R.id.textViewStatus)).setText(message);
	}

	void alertTimeout() {
		handler.post(new Runnable() {
			public void run() {
				Toast.makeText(UserActivity.this, R.string.timeout, Toast.LENGTH_LONG).show();
			}
		});
	}
