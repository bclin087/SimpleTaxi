...
public class DriverActivity extends Activity {
	...
	public void onPickup(View view) {
		...
		// 完成任務
			...
			((Button)findViewById(R.id.buttonPickUp)).setText(R.string.pickup);
			showUserInfoArea(false);
			isOnDuty = false;
			curTxnid = "";
		}
	...
	void requestAPIFunction1() {
		if(currentLocation != null) {
			String url = String.format(Locale.TAIWAN, api_url+"?f=1&p1=%s&p2=%f&p3=%f",
					driverId,
	...
	void requestAPIFunction2() {
		if(currentLocation != null) {
			String url = String.format(Locale.TAIWAN, api_url+"?f=2&p1=%s&p2=%s&p3=%f&p4=%f",
					driverId,
					curTxnid,
					currentLocation.getLatitude(),
					currentLocation.getLongitude()
					);
			Log.v("requestAPIFunction2", url);
			JsonObjectRequest jsObjRequest = new JsonObjectRequest
					(Request.Method.GET, url, null, new Response.Listener<JSONObject>() {
						...
						public void onResponse(JSONObject response) {
							...
							if(result) {
								alertGotBusiness();
								((Button)findViewById(R.id.buttonPickUp)).setText(R.string.arrived);
								((Button)findViewById(R.id.buttonPickUp)).setEnabled(true);
								findViewById(R.id.textViewAlertLabel).setVisibility(View.GONE);
							}
							// 沒接到
							else {
								isOnDuty = false;
	...
	void showUserInfoArea(boolean on) {
		if(on) {
			...
			findViewById(R.id.buttonPickUp).setEnabled(true);
		}
		else {
	...
	void alertNoBusiness() {
		Toast.makeText(this, getResources().getString(R.string.no_business), Toast.LENGTH_LONG).show();
	}
}
